...
final class RY_ECPay_Shipping {
	...
	public static function init() {
		...
		add_filter( 'woocommerce_reports_order_statuses', array(__CLASS__, 'include_custom_order_status_to_reports'), 20, 1 );
		...
		if( 'yes' === get_option(RY_WT::$option_prefix . 'ecpay_shipping_cvs', 'yes') ) {
			...
			add_filter('woocommerce_checkout_fields', array(__CLASS__, 'add_cvs_info'));
	...
	public static function log($message, $level = 'info') {
		if( self::$log_enabled ) {
			...
			self::$log->log($level, $message, array('source' => 'ry_ecpay_shipping'));
		}
	}
	...
	public static function include_custom_order_status_to_reports($order_statuses) {
		array_push($order_statuses, 'wc-ry-at-cvs');
		return $order_statuses;
	}
	...
	public static function shipping_chouse_cvs() {
		...
		if( array_key_exists($method, self::$support_methods) ) {
			wc_get_template('cart/cart-chouse-cvs.php', array(), '', RY_WT_PLUGIN_DIR . 'templates/');
	...
	public static function is_need_checkout_fields() {
		...
		if( $used_cvs ) {
			add_filter('woocommerce_checkout_fields', array(__CLASS__, 'fix_add_cvs_info'), 15);
		} else {
			add_filter('woocommerce_checkout_fields', array(__CLASS__, 'fix_noin_add_cvs_info'), 15);
		}
	}
